...
interface IPriceSource {
    ...
    ) external view returns (uint[] memory prices, uint[] memory timestamps);
    ...
    function getInvertedPriceInfo(
        address ofAsset
    ) external view returns (uint price, uint decimals);

    function getReferencePriceInfo(
        address _base,
        address _quote
    ) external view returns (uint referencePrice, uint decimal);
    ...
    function getOrderPriceInfo(
        ...
        uint sellQuantity,
        uint buyQuantity
    ) external view returns (uint orderPrice);

    function existsPriceOnAssetPair(
        address sellAsset,
        address buyAsset
